...
const NODE_PLUGIN_RULE_NAMES = getPluginRuleNames('node')
...
describe.each(NODE_PLUGIN_RULE_NAMES)('%s rule', (ruleName) => {
  it('is configured', () => {
...
describe.each(Object.entries(rules))('%s definition', (ruleName, rule) => {
  it('matches an existing rule', () => {
    expect(NODE_PLUGIN_RULE_NAMES).toContain(ruleName)
  })
