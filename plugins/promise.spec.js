...
const { rules } = require('./promise')

const PROMISE_PLUGIN_RULE_NAMES = getPluginRuleNames('promise')
...
describe.each(PROMISE_PLUGIN_RULE_NAMES)('%s rule', (ruleName) => {
  it('is configured', () => {
...
describe.each(Object.entries(rules))('%s definition', (ruleName, rule) => {
  it('matches an existing rule', () => {
    expect(PROMISE_PLUGIN_RULE_NAMES).toContain(ruleName)
  })
